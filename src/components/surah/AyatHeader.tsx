...
type Props = {
   nextSurat: Surat | false;
   beforeSurat: Surat | false;
   suratName: string;
   suratId: number;
   isLoading: boolean;
};
   ...
   return (
      <header className="fixed top-0 left-0 z-40 flex h-20 w-full items-center justify-center bg-white px-3 drop-shadow-sm">
         <div className="grid w-full grid-cols-3 items-center justify-items-center">
            {beforeSurat != false ? (
               <Link to={`/surah/${beforeSurat?.nomor}`}>
                  {!isLoading ? (
            ...
            <p className="font-medium text-slate-700">{suratName}</p>
            {nextSurat != false ? (
               <Link to={`/surah/${nextSurat?.nomor}`}>
                  {!isLoading ? (
                     <div className="flex items-center gap-1">
                        <p className="font-medium text-slate-700">
                           {nextSurat?.namaLatin}
